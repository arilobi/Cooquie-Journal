...
def update_entry(entry_id):
    ...
    print(f"🚨 DEBUG UPDATE ROUTE CALLED 🚨")
    print(f"JWT Identity: '{current_user_id}' (type: {type(current_user_id)})")
    print(f"Entry ID requested: {entry_id}")
    
    # First, find ANY entry with this ID
    any_entry = Entry.query.filter_by(id=entry_id).first()
    print(f"Entry exists in DB: {any_entry is not None}")
    
    if any_entry:
        print(f"Entry user_id: {any_entry.user_id} (type: {type(any_entry.user_id)})")
        print(f"Match check: {any_entry.user_id} == {int(current_user_id)} = {any_entry.user_id == int(current_user_id)}")
    
    # Now try with user filter
    entry = Entry.query.filter_by(id=entry_id, user_id=int(current_user_id)).first()
    print(f"Entry found with user filter: {entry is not None}")
    
    if not entry:
        print("❌ FAILED: No entry found with user filter")
        return jsonify({"error": "Entry doesn't exist!"}), 404

    print("✅ SUCCESS: Entry found, proceeding with update...")
